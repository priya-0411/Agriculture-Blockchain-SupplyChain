    ...
    <div className="farmer-dashboard">
      ...
      <div className="product-grid">
        {products.map((p, index) => {
          const idOrIndex = p.id || index;
          const trackUrl = `${window.location.origin}/track/${idOrIndex}`;
          return (
          <div key={index} className="product-card">
            <img src={p.image} alt="crop" className="crop-img" />
            <h3>{p.cropName}</h3>
            ...
            <div className="qr-section">
              <div className="qr-box">
                <QRCodeCanvas
                  ...
                  includeMargin={true}
                />
                <p className="qr-text">Scan to Trace Product</p>
              </div>
            </div>

            <div className="card-buttons">
              <button className="track-btn" onClick={() => navigate(`/track/${idOrIndex}`)}>
                Track Product
              </button>
